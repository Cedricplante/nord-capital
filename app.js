...
function updateEntriesTotal(){
  ...
  // Les montants (e.amount) sont saisis en amount_currency, pas en currency (prix) —
  // il faut convertir avec la bonne devise sinon le total affiché est faussé.
  if(el)el.textContent=fmtC(total,stratDraft.amount_currency||getStratCurrency(stratDraft));
}
...
// ─── UNIFORMISATION DÉCIMALE : point uniquement, jamais de virgule ──
// Demandé par Cédric — toute virgule tapée dans un champ numérique doit
// devenir automatiquement un point. Délégation sur document pour couvrir
// aussi les champs créés dynamiquement (paliers stratégie, modals, etc.).
function isDecimalInput(t){
  return t&&t.tagName==='INPUT'&&(t.type==='number'||t.getAttribute('inputmode')==='decimal');
}
document.addEventListener('beforeinput',function(e){
  if(e.data===','&&isDecimalInput(e.target)){
    e.preventDefault();
...
// Filet de sécurité (paste, autofill, navigateurs sans support beforeinput) :
// si une virgule se glisse quand même dans la valeur, on la convertit.
document.addEventListener('input',function(e){
  const t=e.target;
  if(isDecimalInput(t)&&typeof t.value==='string'&&t.value.includes(',')){
    const start=t.selectionStart,end=t.selectionEnd;
    t.value=t.value.replace(/,/g,'.');
    if(start!=null&&t.setSelectionRange){try{t.setSelectionRange(start,end);}catch(err){}}
  }
});
